...
def perfect_square(score):
    """Returns whether the score is a perfect square.

    score: The total score a player got.
    """
    return (score ** 0.5) % 1 == 0

def next_perfect_square(score):
    """Return score after applied Square Swine rule: (sqrt(score) + 1) ^ 2
    
    score: The total score before applied Square Swine
    """
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    i, j = 0, 0
    fixedNum = strategy(5, 5)
    while i <= goal:
        while j <= goal:
            if strategy(i, j) != fixedNum:
                return False
            j += 1
        i += 1
        j = 0
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    result = 1
    while i <= 10:
        avg_dice = make_averaged(roll_dice, total_samples)(i, dice)
        if avg_dice > max:
            max = avg_dice
            result = i
        i += 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    nxt = tail_points(opponent_score) + score
    if perfect_square(nxt):
        nxt = next_perfect_square(nxt)
...
def get_perfect_number(dice=six_sided, total_samples=1000):
    """Return the number of dice (1 to 10) that gives the highest average turn score
    by calling roll_dice with the provided DICE a total of TOTAL_SAMPLES times and the
    turn score.
    Assume that the dice always return positive outcomes.
    """
    avg_returns = [0 for _ in range(1, 11)]
    for i in range(len(avg_returns)):
        avg_returns[i] = make_averaged(roll_dice, total_samples)(i + 1, dice)
    return avg_returns

avg_returns = get_perfect_number()

def final_strategy(score, opponent_score):
    """Retrun perfect roll numbers.
    apply common_num from max_scoring_num_rolls
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    best_num = 1
    for i in range(len(avg_returns)):
        dist = abs((GOAL - score) - avg_returns[i])
        if dist < closest:
            closest = dist
            best_num = i + 1
    return square_strategy(score, opponent_score, 12, best_num) # Remove this line once implemented.
    # END PROBLEM 12
