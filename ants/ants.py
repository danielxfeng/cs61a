...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if not hasattr(self, "has_doubled"):
            self.damage *= 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        the_nearest_bee = None
        curr_place = self.place
        dist = 0
        while (not the_nearest_bee) and curr_place and (not curr_place.is_hive) and (dist <= self.upper_bound):
            if dist >= self.lower_bound:
                the_nearest_bee = random_bee(curr_place.bees)
            curr_place = curr_place.entrance
            dist += 1
        return the_nearest_bee  # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        [bee.reduce_health(amount) for bee in self.place.bees[:]]
        if amount >= self.health:
            [bee.reduce_health(self.damage) for bee in self.place.bees[:]]
        super().reduce_health(amount)
        # END Problem 5

class WallAnt(Ant):
    """A WallAnt that only have larger health to protect the ant base."""
...
class HungryAnt(Ant):
    """A HungryAnt that can eat a bee in its place, but should pause 3 turns before the next meal."""
...
class ScubaThrower(ThrowerAnt):
    """A thrower ant but can survive in Water"""
    ...
    def insects_in_front(self):
        ...
        while curr_place and not curr_place.is_hive:
            for bee in curr_place.bees:
                insects[bee] = i
            if curr_place.ant and curr_place.ant.name != "Laser":
                insects[curr_place.ant] = i
            i += 1
            curr_place = curr_place.entrance
        return insects
        # END Problem Optional 2
